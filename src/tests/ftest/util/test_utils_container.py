...
class TestContainer(TestDaosApiBase):
    ...
    def get_params(self, test):
        ...
        # Labels only supported with daos command (no pydaos support)
        if self.label.value and self.control_method.value != self.USE_DAOS:
            self.label.update(None, 'label')
        ...
        if self.container:
            ...
            if self.container.attached:
                ...
                if self.control_method.value == self.USE_API:
                    ...
                    raise DaosTestError(
                        "Undefined control_method: {}".format(self.control_method.value))
